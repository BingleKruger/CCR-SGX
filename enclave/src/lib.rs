...
//The threshold is set to supress data less than 1 000 entries
fn summary(numbers: &mut sgx_tstd::vec::Vec<f64>) {
    ...
    if numbers.len() < threshold {
        ...
        println!("This query is suppressed");
    } else {
...
//The function returns a result
fn seal_method_one(unsealed_vector: sgx_tstd::vec::Vec<Entry>, sealed_cipher_vec_pointer: *mut u8) -> Result<(*mut u8, usize), sgx_status_t> {
...
//The function returns a result
fn unseal_method_one(sealed_log: *mut u8, sealed_log_size: usize) ->  Result<sgx_tstd::vec::Vec<Entry>, sgx_status_t> {
...
fn seal_method_two(unsealed_vector: sgx_tstd::vec::Vec<Entry>) -> sgx_status_t {
    let encoded_vec = serde_cbor::to_vec(&unsealed_vector).unwrap();
    let encoded_slice = encoded_vec.as_slice();
...
fn unseal_method_two() -> Option<Vec<Entry>> {
    let mut data_vec: Vec<u8> = Vec::new();

    let data_json_str : Option<Vec<Entry>> = match SgxFile::open(DATAFILE) {
        Ok(mut f) => match f.read_to_end(&mut data_vec) {
            Ok(len) => {
                println!("Read {} bytes from Key file", len);
                println!("Data unsealed successfully");
                let clients_unsealing_two:sgx_tstd::vec::Vec<Entry> = serde_cbor::from_slice(&data_vec).unwrap();
...
pub const SCRATCH_PAD_SIZE: usize = 5000 * MEGA_BYTE;
...
// 6 - Panel data regression examples
#[no_mangle]
...
pub extern "C" fn seal_sgxdata_eg() -> sgx_status_t {
    ...
    let json_sealing_method_one =  sgx_tstd::path::Path::new("../data/POC_1000000.json");
    let file_sealing_method_one =  sgx_tstd::untrusted::fs::File::open(json_sealing_method_one).expect("file not found");
    let clients_sealing_one:sgx_tstd::vec::Vec<Entry> = serde_json::from_reader(file_sealing_method_one).expect("Error while reading or parsing");
    ...
    let mut ID: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Name: Vec<String> = sgx_tstd::vec::Vec::new();
    let mut Surname: Vec<String> = sgx_tstd::vec::Vec::new();
    let mut Sex: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Education: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Marriage: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Age: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Phone: Vec<String> = sgx_tstd::vec::Vec::new();
    let mut Email: Vec<String> = sgx_tstd::vec::Vec::new();
    let mut Street: Vec<String> = sgx_tstd::vec::Vec::new();
    let mut City: Vec<String> = sgx_tstd::vec::Vec::new();
    let mut Postal: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Loan: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Interest: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Term: Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut Income: Vec<f64> = sgx_tstd::vec::Vec::new();

    for i in 0..unsealed_data_one.len() {
        ID.push(unsealed_data_one[i].ID);
        Name.push(unsealed_data_one[i].Name.clone());
        Surname.push(unsealed_data_one[i].Surname.clone());
        Sex.push(unsealed_data_one[i].Sex);
        Education.push(unsealed_data_one[i].Education);
        Marriage.push(unsealed_data_one[i].Marriage);
        Age.push(unsealed_data_one[i].Age);
        Phone.push(unsealed_data_one[i].Phone.clone());
        Email.push(unsealed_data_one[i].Email.clone());
        Street.push(unsealed_data_one[i].Street.clone());
        City.push(unsealed_data_one[i].City.clone());
        Postal.push(unsealed_data_one[i].Postal);
        Loan.push(unsealed_data_one[i].Loan);
        Interest.push(unsealed_data_one[i].Interest);
        Term.push(unsealed_data_one[i].Term);
        Income.push(unsealed_data_one[i].Income);
    }

    println!();
    println!("This is a sample of the unsealed data (Income):");
    summary(&mut Income);
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    sgx_status_t::SGX_SUCCESS
}
...
pub extern "C" fn seal_sgxfile_eg() -> sgx_status_t {
    ...
    let json_sealing_method_two =  sgx_tstd::path::Path::new("../data/POC_1000000.json");
    let file_sealing_method_two =  sgx_tstd::untrusted::fs::File::open(json_sealing_method_two).expect("file not found");
    let clients_sealing_two:sgx_tstd::vec::Vec<Entry> = serde_json::from_reader(file_sealing_method_two).expect("Error while reading or parsing");
...
pub extern "C" fn unseal_sgxfile_eg() -> sgx_status_t {
    ...
    for i in 0..unsealed_data_two.len() {
        ID.push(unsealed_data_two[i].ID);
        Name.push(unsealed_data_two[i].Name.clone());
        Surname.push(unsealed_data_two[i].Surname.clone());
        Sex.push(unsealed_data_two[i].Sex);
        Education.push(unsealed_data_two[i].Education);
        Marriage.push(unsealed_data_two[i].Marriage);
        Age.push(unsealed_data_two[i].Age);
        Phone.push(unsealed_data_two[i].Phone.clone());
        Email.push(unsealed_data_two[i].Email.clone());
        Street.push(unsealed_data_two[i].Street.clone());
        City.push(unsealed_data_two[i].City.clone());
        Postal.push(unsealed_data_two[i].Postal);
        Loan.push(unsealed_data_two[i].Loan);
        Interest.push(unsealed_data_two[i].Interest);
        Term.push(unsealed_data_two[i].Term);
        Income.push(unsealed_data_two[i].Income);
    }
...
pub extern "C" fn seal_sgxfile_ccr() -> sgx_status_t {
    ...
    let json_sealing_ccr =  sgx_tstd::path::Path::new("../data/POC_1000000.json");
...
#[no_mangle]
pub extern "C" fn ccr_poc(some_string_rows: *const u8, some_len_rows: usize) -> sgx_status_t {

    //Receive the command from the untrusted code
    let str_slice_rows = unsafe { slice::from_raw_parts(some_string_rows, some_len_rows) };
    let str_slice_rows = str::from_utf8(&str_slice_rows).unwrap().parse::<f64>().unwrap();

    //Default path
    let mut json_file_path = sgx_tstd::path::Path::new("../data/POC_1000.json");
    let loops: u8 = 11;

    if str_slice_rows == 1.0 {
        // do nothing
    } else if str_slice_rows == 2.0 {
        json_file_path = sgx_tstd::path::Path::new("../data/POC_10000.json");
    } else if str_slice_rows == 3.0 {
        json_file_path = sgx_tstd::path::Path::new("../data/POC_100000.json");
    } else if str_slice_rows == 4.0 {
        json_file_path = sgx_tstd::path::Path::new("../data/POC_1000000.json");
    } else {
        println!("[-] Error: Invalid operation selected");
        return sgx_status_t::SGX_ERROR_INVALID_PARAMETER;
    }

    let mut json_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut parse_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut vec_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut summary_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut mean_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut median_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut sd_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut seal_1_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut unseal_1_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut seal_2_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
    let mut unseal_2_time: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();

    //Loop 10 times
    for _j in 1..loops {
        let json_time_start = Instant::now();
        let file = sgx_tstd::untrusted::fs::File::open(json_file_path).expect("File not found");
        json_time.push(json_time_start.elapsed().as_secs_f64());
        println!("File opened");

        let parse_time_start = Instant::now();
        let entries:sgx_tstd::vec::Vec<Entry> = serde_json::from_reader(file).expect("Error while reading or parsing");
        parse_time.push(parse_time_start.elapsed().as_secs_f64());
        println!("Parsed");

        if str_slice_rows == 7.0 {
            // do not use seal method 1
        } else {
            // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            let file_sealing_method_one =  sgx_tstd::untrusted::fs::File::open(json_file_path).expect("file not found");
            let clients_sealing_one:sgx_tstd::vec::Vec<Entry> = serde_json::from_reader(file_sealing_method_one).expect("Error while reading or parsing");
            ...
            let seal_1_time_start = Instant::now();
            let sealed_data_tuple = match seal_method_one(clients_sealing_one, sealedcipher_vec_pointer) {
                Ok(x) => x,
                Err(ret) => {
                    return ret
            ...
            seal_1_time.push(seal_1_time_start.elapsed().as_secs_f64());
            println!();
            println!("Seal method one");

            // Unsealing with SgxSealedData
            let unseal_1_time_start = Instant::now();
            let _unsealed_data_one = match unseal_method_one(sealed_data_tuple.0, sealed_data_tuple.1) {
                Ok(x) => x,
                Err(ret) => {
                    return ret
                },
            };
            unseal_1_time.push(unseal_1_time_start.elapsed().as_secs_f64());
            println!();
            println!("Unseal method one");

            // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        }

        let file_sealing_method_two =  sgx_tstd::untrusted::fs::File::open(json_file_path).expect("file not found");
        let clients_sealing_two:sgx_tstd::vec::Vec<Entry> = serde_json::from_reader(file_sealing_method_two).expect("Error while reading or parsing");

        let seal_2_time_start = Instant::now();
        seal_method_two(clients_sealing_two);
        seal_2_time.push(seal_2_time_start.elapsed().as_secs_f64());
        println!();
        println!("Seal method two");

        let unseal_2_time_start = Instant::now();
        let _unsealed_data_two = unseal_method_two().unwrap();
        unseal_2_time.push(unseal_2_time_start.elapsed().as_secs_f64());
        println!();
        println!("Unseal method two");
        // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        let vec_time_start = Instant::now();
        let mut field_test: sgx_tstd::vec::Vec<f64> = sgx_tstd::vec::Vec::new();
        for entry in entries {
            field_test.push(entry.Loan);
        }
        vec_time.push(vec_time_start.elapsed().as_secs_f64());


        let summary_time_start = Instant::now();
        summary(&mut field_test);
        summary_time.push(summary_time_start.elapsed().as_secs_f64());

        //Create a clone of the numbers vector received to ensure it does not change
        let mut numbers_cloned: sgx_tstd::vec::Vec<f64> = field_test.clone();

        //Sort the numbers_cloned from the smallest to the largest value
        let median_time_start = Instant::now();
        numbers_cloned.sort_by(|a, b| a.partial_cmp(b).unwrap());


        //Find the index for Q2
        //Assume n = number of observations
        let Q2_Index = 0.5 * (numbers_cloned.len() as i64 +1) as f64;
        let _Q2: f64 = mean(&vec![numbers_cloned[(Q2_Index.floor() -1.0) as usize], numbers_cloned[(Q2_Index.ceil() -1.0) as usize]]) as f64;
        median_time.push(median_time_start.elapsed().as_secs_f64());


        //Calculate the mean of the vector
        let mean_time_start = Instant::now();
        let data_mean = mean(&numbers_cloned);
        mean_time.push(mean_time_start.elapsed().as_secs_f64());


        //Calculate the sd of the vector
        let sd_time_start = Instant::now();
        let variance = numbers_cloned.iter().map(|value| {
            let diff = data_mean - (*value as f64);
            diff * diff
        }).sum::<f64>() / (numbers_cloned.len() as f64 - 1.0);
        let _SD = variance.sqrt();
        sd_time.push(sd_time_start.elapsed().as_secs_f64());


    }
    println!("Time to open json file: {:?}", json_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &json_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(), mean(&json_time) ,json_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to parse data: {:?}", parse_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &parse_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&parse_time) ,parse_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to seal data with method 1: {:?}", seal_1_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &seal_1_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&seal_1_time) ,seal_1_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to unseal data with method 1 {:?}", unseal_1_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &unseal_1_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&unseal_1_time) ,unseal_1_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to seal data with method 2: {:?}", seal_2_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &seal_2_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&seal_2_time) ,seal_2_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to unseal data with method 2 {:?}", unseal_2_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &unseal_2_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&unseal_2_time) ,unseal_2_time.iter().copied().fold(f64::NAN, f64::max));
    println!();


    println!("Time to create vector: {:?}", vec_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &vec_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&vec_time) ,vec_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to calculate summary statistics is: {:?}", summary_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &summary_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&summary_time) ,summary_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to calculate mean: {:?}", mean_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &mean_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&mean_time) ,mean_time.iter().copied().fold(f64::NAN, f64::max));
    println!();


    println!("Note this includes time to sort the vector:");
    println!("Time to calculate median: {:?}", median_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &median_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&median_time) ,median_time.iter().copied().fold(f64::NAN, f64::max));
    println!();

    println!("Time to calculate sd: {:?}", sd_time);
    println!("Min: {:.7}  Average: {:.7}  Max: {:.7}", &sd_time.clone().into_iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() , mean(&sd_time) ,sd_time.iter().copied().fold(f64::NAN, f64::max));
    println!();
    sgx_status_t::SGX_SUCCESS
}
